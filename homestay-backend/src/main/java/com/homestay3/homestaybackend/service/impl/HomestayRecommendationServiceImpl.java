...
public class HomestayRecommendationServiceImpl implements HomestayRecommendationService {
    ...
    @Override
    // @Cacheable(value = "recommendedHomestays", key = "#limit") // 暂时禁用缓存进行调试
    public List<HomestayDTO> getRecommendedHomestays(int limit) {
        ...
        if (strictResults.size() < limit) {
            ...
            for (RecommendationScore score : relaxedResults) {
                if (finalResults.size() >= limit * 2) break; // 预留多一些供多样化算法选择
            ...
            if (finalResults.size() < limit && allHomestays.size() > 0) {
                ...
                for (Homestay homestay : allHomestays) {
                    if (finalResults.size() >= limit * 2) break;
        ...
        switch (request.getRecommendationType()) {
            ...
            case PERSONALIZED:
                return request.getUserId() != null ? 
                    getPersonalizedRecommendations(request.getUserId(), 20) : 
    ...
    @Override
    @CacheEvict(value = {"popularHomestays", "recommendedHomestays", "userRecommendations"}, allEntries = true)
    ...
    private HomestayDTO convertToDTO(Homestay homestay) {
        try {
            ...
            // dto.setAmenities(homestay.getAmenities() != null ? homestay.getAmenities() : Collections.emptyList()); // 这需要转换为AmenityDTO，先跳过
    ...
    private List<PersonalizedScore> diversifyPersonalizedResults(
        ...
        if (preference.getPreferredCities() == null || 
            ...
            for (PersonalizedScore score : scores) {
                if (result.size() >= limit * 3 / 4) break;
                if (result.contains(score)) continue;
    ...
    private static class UserPreference {
        ...
        // getters and setters
        public Integer getPreferredMinPrice() { return preferredMinPrice; }
        public void setPreferredMinPrice(Integer preferredMinPrice) { this.preferredMinPrice = preferredMinPrice; }
        
        public Integer getPreferredMaxPrice() { return preferredMaxPrice; }
        public void setPreferredMaxPrice(Integer preferredMaxPrice) { this.preferredMaxPrice = preferredMaxPrice; }
        ...
        public List<String> getPreferredPropertyTypes() { return preferredPropertyTypes; }
        public void setPreferredPropertyTypes(List<String> preferredPropertyTypes) { this.preferredPropertyTypes = preferredPropertyTypes; }
        
        public List<String> getPreferredProvinces() { return preferredProvinces; }
        public void setPreferredProvinces(List<String> preferredProvinces) { this.preferredProvinces = preferredProvinces; }
        ...
        public double getMinRatingPreference() { return minRatingPreference; }
        public void setMinRatingPreference(double minRatingPreference) { this.minRatingPreference = minRatingPreference; }
    }
} 