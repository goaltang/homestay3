...
public class AlipayGateway implements PaymentGateway {
    ...
    public AlipayGateway(PaymentConfig paymentConfig) {
        ...
        alipayConfig.setAlipayPublicKey(paymentConfig.getAlipay().getPublicKey());
        ...
        try {
            ...
            throw new RuntimeException("支付宝网关初始化失败", e);
        }

        log.info("支付宝网关初始化成功，APPID: {}, 网关URL: {}, 连接超时: {}ms, 读取超时: {}ms",
    ...
    public PaymentResponse createQRCodePayment(PaymentRequest request) {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                log.error("支付宝创建扫码支付订单未知异常，异常类型: {}, 异常信息: {}, 尝试次数: {}/{}",
                        e.getClass().getName(), e.getMessage(), attempt, maxRetries, e);
                ...
                return PaymentResponse.builder()
                        .success(false)
                        .message("系统异常: " + e.getClass().getSimpleName() + " - " + e.getMessage())
                        .errorCode("SYSTEM_ERROR")
                        .build();
            }
        }
    ...
    public RefundResponse processRefund(RefundRequest request) {
        ...
        try {
            ...
            bizContent.put("refund_reason", request.getRefundReason() != null ? request.getRefundReason() : "正常退款");
